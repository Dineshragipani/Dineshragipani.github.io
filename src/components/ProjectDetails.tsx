import React from 'react'
import {motion} from "motion/react";

interface ProjectDetailsProps {
  title: string;
  description: string;
  subDescription: string[];
  href: string;
  image: string;
  tags: {
    id: string | number;
    name: string;
    image?: string;
  }[];
  closeModel: () => void;
}

const ProjectDetails: React.FC<ProjectDetailsProps> = ({title,
     description, 
     subDescription, 
     href, 
     image, 
     tags,
    closeModel}) => {
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center w-full overflow-hidden backdrop-blur-sm'>
        <motion.div className='relative max-w-2xl border shadow-sm rounded-2xl bg-gradient-to-l from-midnight to-navy border-white/10'
        initial={{opacity:0, scale:0.5}}
        animate={{opacity:1, scale:1}}>
            <button
            onClick={closeModel}
            className='absolute p-2 rounded-sm top-5 right-5 bg-midnight hover:bg-gray-500 '>
                <img src='assets/close.svg' alt='Close' className='w-6 h-6'/>
            </button>
            <img src={image} alt='title' className='w-4 rounded-t-2xl'/>
            <div className='p-5'>
                <h5 className='mb-2 text-2xl font-bold text-white'>{title}</h5>
                <p className='mb-3 font-normal text-neutral-400'>{description}</p>
                {subDescription.map((subDec,index)=>(<p className='mb-3 font-normal text-neutral-400' key={index}>{subDec}</p>))}
                <div className='flex items-center justify-between mt-4'>
                    <div className='flex gap-3 '>
                        {tags.map((tag)=>(<img key={tag.id} alt={tag.name} className='rounded-lg size-10 hover-animation'/>))}
                    </div>
                    <a className='inline-flex items-center gap-1 font-medium cursor-pointer hover-animation'>
                        View Projects
                        <img src='assets/arrow-up.sg'className='size-4' href={href}/>
                    </a>
                </div>
            </div>
        </motion.div>
    </div>
  )
}

export default ProjectDetails